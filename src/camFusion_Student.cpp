...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    for (auto match : kptMatches)
    {
        ...
        {
            distance = sqrt(pow(currKp.pt.x - prevKp.pt.x, 2) + pow(currKp.pt.y - prevKp.pt.y, 2));
    ...
    for (float dis : distances) {
        standardDeviation += pow(dis - meanDistance, 2);
    }

    standardDeviation = sqrt(standardDeviation / distances.size());   
    ...
    for (auto match : kptMatches)
    {
        ...
        if (boundingBox.roi.contains(currKp.pt) && (fabs(distance - meanDistance) < standardDeviation))
        {
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
        ...
        for (auto box : prevFrame.boundingBoxes)
        {
            if (box.roi.contains(prevKp.pt))
            {
                ...
                for (auto box : currFrame.boundingBoxes)
                {
                    ...
                    {
                        currBoxID = box.boxID;
                        boxpairs.insert(pair<int, int>(currBoxID, prevBoxID));
                        prevmaxBoxID = std::max(prevmaxBoxID, prevBoxID);
                        currmaxBoxID = std::max(currmaxBoxID, prevBoxID);
                    }
                }
            }   
        }
    ...
    for(int i = 0; i <= currmaxBoxID+1; i++)
        currBoxIds.push_back(i);

    for (int currID : currBoxIds)
    {
        auto resultpairs = boxpairs.equal_range(currID);
        vector<int> countIDs(prevmaxBoxID+1);
        
        for (auto it = resultpairs.first; it != resultpairs.second; ++it)
        {
            countIDs[it->second] += 1;
        }

        int prevID = std::distance(countIDs.begin(), std::max_element(countIDs.begin(), countIDs.end())); 
